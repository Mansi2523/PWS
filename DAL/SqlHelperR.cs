...
namespace DAL1
{
    ...
    public sealed class SqlHelper1
    {
        ...
        private static string SetConnection()
        {
            ...
            return Convert.ToString(ConfigurationManager.AppSettings["ConnectionString1"]);
        }
        ...
        /// <summary>
        /// Execute a SqlCommand (that returns no resultset and takes no parameters) against the database default Connection string
        /// </summary>
        /// <param name="commandType">either store procedure or text</param>
        /// <param name="commandText">if store procedure then sp name else command text</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Execute a SqlCommand (that returns no resultset) against the database specified in the default connection string
        /// using the provided parameters.
        /// </summary>
        /// <param name="commandType">the CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">the stored procedure name or T-SQL command</param>
        ...
        public static int ExecuteNonQuery(string spName, params object[] parameterValues)
        {
            ...
            {
                //pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                SqlParameter[] commandParameters = SqlHelperParameterCache.GetSpParameterSet(SetConnection(), spName);

                //assign the provided values to these parameters based on parameter order
                AssignParameterValues(commandParameters, parameterValues);

                //call the overload that takes an array of SqlParameters
                return ExecuteNonQuery(SetConnection(), CommandType.StoredProcedure, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
        ...
        /// <summary>
        /// Execute a SqlCommand (that returns a resultset and takes no parameters) against the database default connection string
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ...
        public static int UpdateDataset(DataSet ds, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            using (SqlConnection cn = new SqlConnection(SetConnection()))
            {
                ...
                //call the overload that takes a connection in place of the connection string
                UpdateDataset(ds, cn, commandType, commandText, commandParameters);
                tx.Commit();
                return 1;
            }
        }
        public static int UpdateDataset(DataSet ds, SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            ...
            /*da.Fill(ds, "tbl_City");
            cmd.CommandTimeout = 400;
            ds.Tables[0].Rows[0]["varCityName"] = "12345";
            DataSet modifiedDS = ds.GetChanges(DataRowState.Modified);
            da.Update(modifiedDS, "tbl_City");*/
        ...
        ///// <summary>
        ///// Execute a SqlCommand (that returns a resultset and takes no parameters) against the database use default connection string
        ///// </summary>
        ///// <remarks>
        ///// e.g.:  
        /////  DataTable dt = ExecuteDataset(CommandType.StoredProcedure, "GetOrders");
        ///// </remarks>
        ///// <param name="commandType">the CommandType (stored procedure, text, etc.)</param>
        ///// <param name="commandText">the stored procedure name or T-SQL command</param>
        ///// <returns>a dataTable containing the resultset generated by the command</returns>
        ...
        {
            //create & open a SqlConnection
            SqlConnection cn = new SqlConnection(SetConnection());
            cn.Open();
        ...
        public static SqlDataReader ExecuteReader(string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //call the overload that takes an array of SqlParameters
                return ExecuteReader(SetConnection(), CommandType.StoredProcedure, spName, commandParameters);
            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteReader(SetConnection(), CommandType.StoredProcedure, spName);
            }
        }
